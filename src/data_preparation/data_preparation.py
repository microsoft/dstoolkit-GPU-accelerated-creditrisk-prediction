...
class DataPreparation:
    ...
    def read_acquisition_files(self) -> DataFrame:
        ...
        for yr in self.years:
            for qtr in [1, 2, 3, 4]:
                print(
                    "Reading the file Acquisition_" + str(yr) + "Q" + str(qtr) + ".txt"
                )
                file_path = os.path.join(
                    self.data_folder_path,
                    "acq/Acquisition_" + str(yr) + "Q" + str(qtr) + ".txt",
                )
                df_acq_qtr = read_csv(
                    file_path, sep="|", names=ACQUISITION_COLS, index_col=False
                )  # dtype=dtype)
    ...
    @timeit
    def merge_acq_per_files(
        self, df_acq: DataFrame, df_delinq4: DataFrame
    ) -> DataFrame:
    ...
    def prepare_credit_risk_data(self) -> DataFrame:
        """Reads Acquisition and Performance files, merges them and creates target.

        Returns:
            DataFrame: Dataframe with target column.
        """
        ...
        print(
            "Dropping the columns - 'index','OrDate','OrLTV','MortInsPerc','RelMortInd','FirstPayment', 'Zip','PropertyState'..."
        )
        df.drop(
            [
                "index",
